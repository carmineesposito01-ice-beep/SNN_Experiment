...
%  (SNN+decode+normalize) e le fasi 1/20/12/23/18 (decode); pv arriva dagli ingressi.
  ...
  Lmain = {
    ...
    '  end'
    '  xprev = x;'
    '  if go                                % nuova inferenza: vai alla fase LATCH'
    '    phase = uint8(30);'
    '  end'
    '  if phase == 30                       % LATCH: snapshot REGISTRATO di ingressi+params -> rompe il'
    '    sL = s; vL = v; dvL = dv; vlL = v_l;'  %  percorso combinatorio ingresso->st_a_iidm (in R17 i params'
    '    pv = cast(p, ''like'', pv);'           %  arrivavano gia'' registrati dal decode; qui li registro qui).'
    '    phase = uint8(10);'
