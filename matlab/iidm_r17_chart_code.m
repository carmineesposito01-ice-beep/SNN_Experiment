...
%  (SNN+decode+normalize) e le fasi 1/20/12/23/18 (decode); pv arriva dagli ingressi.
  ...
  Lmain = {
    ...
    '  persistent pv xprev started acc phase kdiv st alf vlp numl denl ql thl dA dR dQ dB dsq dkbit dns sX sR sQ sk sabv blv ouv acbl qaf qbf sL vL dvL vlL'
    '  if isempty(started)'
    '    pv = cast(p, ''like'', fi(zeros(5,1), 1, 21, 13));'
    '    sL = s; vL = v; dvL = dv; vlL = v_l;'
    ...
    '  end'
    '  xprev = x;'
    '  if phase == 0                        % IDLE: l''edge-detect parte SOLO da fermo. Cosi'' `go` (il'
    '    if go                              %  confronto dei 9 ingressi) gatta SOLO la transizione 0->30,'
    '      phase = uint8(30);'              %  NON gli update di st durante l''inferenza (che dipendono'
    '    end'                               %  dalla phase REGISTRATA) -> rompe il critpath param->go->st_a_iidm.'
    '  elseif phase == 30                   % LATCH: snapshot registrato di ingressi+params'
    '    sL = s; vL = v; dvL = dv; vlL = v_l;'
    '    pv = cast(p, ''like'', pv);'
    '    phase = uint8(10);'
    '  elseif phase == 10                   % SQRT-INIT'
    '    [qaf, qbf] = iidm_ab(pv(:));'
    '    phase = uint8(21);'
    ...
    '    [ouv, alf, vlp] = iidm_prep_a(vlL, false, alf, vlp);'
    '    phase = uint8(17);'
    '  elseif phase == 17                   % OU-B'
    '    alf = iidm_prep_a2(ouv, alf);'
    '    phase = uint8(16);'
    '  elseif phase == 16                   % PREP-B (usa lo snapshot registrato)'
    '    st = iidm_prep_b(sL, vL, dvL, vlL, pv(:), alf, sabv);'
