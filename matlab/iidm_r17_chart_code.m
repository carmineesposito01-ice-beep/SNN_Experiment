...
%  (SNN+decode+normalize) e le fasi 1/20/12/23/18 (decode); pv arriva dagli ingressi.
  ...
  Lmain = {
    ...
    '  elseif phase == 30                   % LATCH: registra i params (pv); s/v/dv/v_l restano live'
    '    pv = cast(p, ''like'', pv);'         %  (tenuti stabili dal driver; usarli diretti evita un livello'
    '    phase = uint8(10);'                  %  di mux dello snapshot sul percorso OU/prep, come in R17).'
