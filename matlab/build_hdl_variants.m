...
%ACCIIDM_M_DESCRIPTION  Description del blocco SP4-M-FSM (visibile in Block Properties).
  L = {
    ...
    ''
    'ARCHITETTURA R17 (hdl_iidm/RESULTS.txt): divisore e radice dell''IIDM = digit-recurrence SEQUENZIALI'
    '(1-2 bit/ciclo) orchestrati da una FSM a stadi, ognuno dei 17 round bit-exact (dmax=0 + parity 0/60000).'
    'Fmax OOC xc7z020 R0 15,7 -> R17 77,9 MHz (+397%), 8387 LUT / 4069 FF / 68 DSP. nfrac=8.'
    '⚠️ 77,9 = OOC reg-reg (NON io-timed): il deployabile e'' inferiore; conta il MARGINE sul control-step'
    '(~358 clock su 800.000 @8 MHz), non il valore assoluto.'
    ''
    'INGRESSI (fisici, fixed >=20 bit fraz. - interporre un Data Type Conversion): s, v, dv (= v - v_l), v_l'
    'USCITA: accel [m/s^2]. Edge-triggered (1 cambio ingressi = 1 inferenza, l''uscita tiene). Self-contained.'
    'Rigenerazione: build_hdl_variants.m (+ reorg_library.m per togliere i singoli assorbiti).'
  };
  d = strjoin(L, newline);
end
