# Arch_Tested/A1_baseline_BPTT_864p/core/network.py
# Generato automaticamente: SOLO classi necessarie per variant 'baseline'.
...
# =================================================================
def build_model(variant='baseline', hidden_size=None, rank=None,
                max_delay=None, **kwargs):
    """Factory ristretta per A1_baseline_BPTT_864p. Solo 'baseline'."""
    v = variant.lower()
    if v == 'baseline':
        return CF_FSNN_Net(hidden_size=hidden_size, rank=rank)
    raise ValueError("Variant '%s' non supportata in A1. Solo: baseline" % variant)
