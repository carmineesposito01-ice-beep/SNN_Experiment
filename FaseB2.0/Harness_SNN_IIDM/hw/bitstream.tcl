# T7b — bitstream PYNQ-Z1 del sistema (PS7 + snniidm_axi_lite + protocol converter) alla frequenza
# DEPLOYABILE misurata dallo sweep (40 MHz: WNS +0,022 ns, WHS +0,033 ns).
...
create_bd_cell -type module -reference snniidm_axi_lite tier0
...
file copy -force [lindex $bit 0] "$OUT/donatello_snn_iidm.bit"
puts "BIT_OK: $OUT/donatello_snn_iidm.bit"
...
if {[llength $hwh] > 0} {
  file copy -force [lindex $hwh 0] "$OUT/donatello_snn_iidm.hwh"
  puts "HWH_OK: $OUT/donatello_snn_iidm.hwh"
} else { puts "BIT ATTENZIONE: .hwh non trovato" }
if {[catch { write_hw_platform -fixed -include_bit -force "$OUT/donatello_snn_iidm.xsa" } m]} {
  puts "BIT ATTENZIONE: XSA non generata: $m"
} else { puts "XSA_OK: $OUT/donatello_snn_iidm.xsa" }
puts "BIT-DONE"
