...
>
> | **SCOPE 2026-08-01** | Il **plotone esce dallo studio su hardware**: P3 su silicio cancellato (N istanze su una scheda *simulano* un plotone, e il software lo fa meglio). P1 e P2 restano come risultati software/RTL. |
> | **P1 — plotone (software)** | mediana head-to-tail sopra 1 a ogni N (1,037 a N=2 → 1,458 a N=16): **non** string-stable in mediana. Ogni collisione misurata e' un `aggressive_cut_in` (gap che crolla a 6,3 m, emergenza per costruzione): nessun altro tipo di scenario collide mai. |
> | **Canale V2X** | La latenza non crea modalita' di guasto nuove, **allarga** quella esistente: senza latenza il taglio aggressivo cede solo in autostrada (3 casi su 99), con 300 ms cede a tutte le velocita' (10 su 11). PDR e Gilbert-Elliott non spostano nulla. |
> | **Difetto riparato** | `utils/platoon_eval.simulate_platoon` non applicava il teletrasporto `cut_in`: 33 scenari su 99 vedevano il leader fermarsi di colpo senza il gap compensativo. Codice CONDIVISO, quindi il difetto riguardava anche altri studi. |
