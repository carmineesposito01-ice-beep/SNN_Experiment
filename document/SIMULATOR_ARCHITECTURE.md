...
| `sim/ui/mix_table.py` | **Qt only** — the reusable mix table (7b UI B1/B2) | `MixTable(QWidget)`: the mix as **rows of real widgets** (`_Row`), not a `QTableWidget`. Extracted from `dataset_page.py` so B2 reuses it (train and val become two instances). The family→source combo **cascades** (`built`→its `_specs`, `preset`→library names, `generator`/`cut_in`→`GENERATOR_PROFILES`); `built` is **disabled with a tooltip** when no spec exists. Quota column is LIVE; `is_valid()` is the total==100% gate; every edit emits **`changed`**. The eye draws `preview_sample` at a **fixed `PREVIEW_SEED`**, title says *"campione"*. `strength` is **injected**. **`with_regime=True`** (B2) adds a **regime combo** per row + the **`cut_in`** family, makes `mix()` return `TrainMixEntry`, and adds a live **`→ finestre`** column via the engine's `training_windows` (train stride 50); the eye maps `cut_in`→`generator` (leader A). Eye glyph `⊙` (7a's `👁` rendered as a blob in Windows' default font) |
| `sim/ui/training_panel.py` | **Qt only** — the Training destination (7b UI B2) | A `MixTable(with_regime=True)` + the training controls: `seed`, **train/val counts**, a **3-mode validation selector** (`_val_sel` → `VAL_MODE_*`, each with a live consequence note; mode 3 shows a **second `MixTable`** for `val_mix()`), the jitter caveat **reworded** (governs the leader, not the params), the **frequency disabled** with its reason (`DT` is in the PINN loss), format fixed to **`.pt`**, a size estimate from the **measured 78.3 B/tick**, an **ETA** from `SECONDS_PER_TRAJ × (n_train+n_val)`, the `python train.py --data_cache …` line, and **Generate + Cancel**. Exposes getters (`mix/n_train/n_val/seed/strength/out_path/val_mode/val_mix`) the app reaches with `build_training_cache`. The eye maps `cut_in`→`generator` (leader A) since `preview_sample` has no `cut_in` branch |
| `sim/ui/dataset_page.py` | **Qt only** — the 5th mode, now a **destination toggle + stack** (7b UI B2) | A `QRadioButton` pair **Analisi \| Training** drives a `QStackedWidget` between today's analysis content (an analysis `MixTable` + the export controls, unchanged) and the `TrainingPanel`. `destination()` reports which; `set_sources` reaches both tables. The analysis getters (`mix/count/seed/strength/k/formats/out_dir`) and `_gen_btn`/`_progress` are unchanged, so the app's `generate_dataset` path is untouched. `_refresh` is the analysis `changed` slot |
