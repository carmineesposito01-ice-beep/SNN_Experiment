...
| `sim/ui/mix_table.py` | **Qt only** — the reusable mix table (7b UI B1) | `MixTable(QWidget)`: the mix as **rows of real widgets** (`_Row`), not a `QTableWidget` — every cell is already an interactive control (two cascading combos, an eye, a spin, a ✕) and a table would mean a delegate per column to get back what a widget row gives free. Extracted from `dataset_page.py` so B2 can reuse it (train and val become two instances). The family→source combo **cascades** (`built`→its `_specs`, `preset`→library names, `generator`→`GENERATOR_PROFILES`); `built` is **disabled with a tooltip** when no spec exists. The quota column is LIVE (`quotas()` on every edit); `is_valid()` is the total==100% gate, and every edit emits **`changed`** — the page listens to re-gate + re-estimate. The eye's popup draws `preview_sample` at a **fixed `PREVIEW_SEED`**, title says *"campione"* (a jittered/generator source is a DISTRIBUTION). `strength` is **injected** (the jitter slider lives on the page). `with_regime` is reserved for B2 (raises `NotImplementedError` for now). Eye glyph is `⊙` (7a's `👁` rendered as a blob in Windows' default font) |
| `sim/ui/dataset_page.py` | **Qt only** — the 5th mode, the Dataset builder (7a plan B; 7b B1 slimmed it) | Holds a **`MixTable`** + the export controls (seed, count, jitter, frequency+`?`, format checkboxes rendered FROM `FORMATS`, size estimate, folder, Generate, progress). `_refresh` is a **`changed` slot**: it gates Generate on `mix.is_valid()` and updates the `≈ MB` estimate. The mix rows/cascade/eye/quota now live in `mix_table.py`; the page reaches `self._mix._rows` only in `_ticks_per_traj`. The page **exposes getters** (`mix/count/seed/strength/k/formats/out_dir`); the APP owns the run — the same split as Meso (`app.py:174`) |
...
**373 across 38 files** (37 `test_sim_*.py` + `tests/test_champion_io.py`); **373 green** at end of 7b UI B1
(the MixTable extraction; the engine was 371). `test_sim_mix_table.py` is the isolated unit for the extracted
table. The pure units (`scenario_export`, `mat_writer`, `jitter`, `dataset_mix`,
