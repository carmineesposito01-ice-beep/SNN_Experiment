...
![Figura 2.1 - Accuratezza per parametro (sx) e confronto predetto-vs-vero (dx, scala log). s0/v0/T sono centrati; a è sottostimato (~0.66 vs 1.10) e b sovrastimato (~1.95 vs 1.50): entrambi spingono verso una frenata più conservativa.](figures_validation/val_accuracy.png)
*Figura 2.1 - Accuratezza per parametro (sx) e confronto predetto-vs-vero (dx, scala log). s0/v0/T sono centrati; a è sottostimato (~0.66 vs 1.10) e b sovrastimato (~1.95 vs 1.50): entrambi spingono verso una frenata più conservativa.*

Perché il bias di a e b è "benevolo" per la sicurezza: sottostimare a significa accelerare più dolcemente; sovrastimare b significa supporre una capacità di frenata maggiore, quindi mantenere margini più ampi. Entrambi puntano nella direzione sicura. La causa strutturale è che a entra nel modello IIDM come un termine saturante (cap min(., a)) e accoppiato a b via sqrt(a*b): la sua finestra di osservabilità è stretta per costruzione, e oltre ~0.65 di NRMSE non scende senza un cambio di modello/loss (target dello studio S4).
